	...
	<head>
        ...
        private void LoadFlvplayer(string fileName)
        {
            ...
            try
            {
                ...
                string ObjectStr = " width=" + '"' + this.MediaPlayerPanel.Width + '"' + " height= " + '"' + this.MediaPlayerPanel.Height + '"' +
                ...
                string Param1Str = " name=" + '"' + "movie" + '"' + " value= " + '"' + playerUrl + '"';
                this.SFPlayer.SetVariable("param", Param1Str);
                string Param2Str = " name=" + '"' + "allowFullScreen" + '"' + " value= " + '"' + "true" + '"';
                this.SFPlayer.SetVariable("param", Param2Str);
                string Param3Str = " name=" + '"' + "FlashVars" + '"' + " value= " + '"' + "flvmov=+"+ fileName + '"';
                this.SFPlayer.SetVariable("param", Param3Str);
                this.SFPlayer.FlashVars =  "flvmov=" + '"' + fileName + '"';
                ...
                this.SFPlayer.SetVariable("embed", EmbedStr);
			...
			*/
                ...
                {        //左ボタン
                    ...
                    if (mouceDownPoint != Point.Empty)
                    {
                        ...
                        dbMsg += ">>dMove=" + dMove;
                        if (draglist.ItemHeight < dMove)
                        {    //一行以上の移動   //mouceDownPoint.X != e.X || mouceDownPoint.Y != e.Y
                            dbMsg += "(" + dragSouceIDP + ")";
                            if (-1 < dragSouceIDP)
                            {
		...
		*/
        ...
        private void PlayListBox_DragEnter(object sender, DragEventArgs e)
        {
            ...
            try
            {
                ...
                {                                //エクスプローラー？ if(dragSouceUrl!= b_dragSouceUrl || b_dragSouceUrl =="")
                    if (DragURLs.Count < 1)
                    {
                        DragURLs = new List<string>();
                        foreach (string item in (string[])e.Data.GetData(DataFormats.FileDrop))
                        {       //エクスプローラーから	http://www.itlab51.com/?p=2904	
                            dbMsg += ",=" + item.ToString();
            ...
            try
            {
                ...
                if (dragFrom == playListBox.Name)
                {
                    ...
                    {                //ドラッグされているデータがstring型か調べる
                        ...
                        {   //Altキーが押されていればLink//"32"はAltキーを表す
